...
class OriginOSApp(MDApp):
    ...
    animation_speed = NumericProperty(0.3)
    wallpaper_path = StringProperty("originos_data/wallpaper_3.png")
    app_widgets = {}
    ...
    def open_app_by_index(self, index):
        home_screen = self.root.get_screen('home')
        if 0 <= index < len(home_screen.ids.app_grid.children):
            self.open_app(home_screen.ids.app_grid.children[-(index + 1)])
    ...
    def populate_home_screen(self, screen):
        ...
        for app_info in apps:
            ...
            screen.ids.app_grid.add_widget(app_icon)

    def open_app(self, icon_widget):
        if self.current_app_window and self.opened_from_icon == icon_widget:
            return

        if icon_widget in self.closing_windows:
            app_window = self.closing_windows.pop(icon_widget)
            Animation.cancel_all(app_window)
            self.current_app_window = app_window
            self.opened_from_icon = icon_widget
        else:
            if self.current_app_window:
                self.close_app(switch_app=True)

            icon_card = icon_widget.ids.icon_card
            app_window = MDCard(
                radius=[20,20,20,20],
                md_bg_color=(1,1,1,1),
                size_hint=(None, None),
                size=icon_card.size,
                pos=icon_card.to_window(*icon_card.pos),
                elevation=0,
                orientation='vertical'
            )
        ...
        home_screen = self.root.get_screen('home')
        nav_bar = home_screen.ids.nav_bar
        target_pos = (0, nav_bar.height)
        target_size = (Window.width, Window.height - nav_bar.height)
    ...
    def _animate_close(self, window_to_close, icon_to_return_to, switch_app):
        ...
        lighten_factor = 0.7
        lighter_color = (r + (1 - r) * lighten_factor, g + (1 - g) * lighten_factor, b + (1 - b) * lighten_factor, a)
        ...
        icon_card.add_widget(MDIcon(
            icon=icon_to_return_to.icon, halign='center', valign='center',
            pos_hint={'center_x': .5, 'center_y': .5}, font_size="30sp"
        ))
        closing_icon_container.add_widget(icon_card)
        window_to_close.add_widget(closing_icon_container)

        icon_card_target = icon_to_return_to.ids.icon_card
        target_pos = icon_card_target.to_window(*icon_card_target.pos)
        target_size = icon_card_target.size
        
        anim = Animation(
            pos=target_pos,
            size=target_size,
            md_bg_color=lighter_color,
            duration=self.animation_speed,
            t='in_quad'
        )

        def on_close_complete(*args):
            if icon_to_return_to in self.closing_windows:
                if window_to_close.parent:
                    window_to_close.parent.remove_widget(window_to_close)
                del self.closing_windows[icon_to_return_to]
    ...
    def close_app(self, *args, switch_app=False):
        if not self.current_app_window:
            return

        window_to_close = self.current_app_window
        icon_to_return_to = self.opened_from_icon
        
        if not switch_app:
            self.closing_windows[icon_to_return_to] = window_to_close
        
        self.current_app_window = None
        self.opened_from_icon = None
        
        self._animate_close(window_to_close, icon_to_return_to, switch_app)
    ...
    def show_wallpaper_dialog(self):
        if not self.wallpaper_dialog:
            ...
            button_box.add_widget(MDBoxLayout())
